...
class Transferencia(Transacao):
    ...
    def registrar(self, conta_origem):
        sucesso_transacao_origem = conta_origem.sacar(self.valor)
        if sucesso_transacao_origem:
...
def extrato_por_periodo(clientes):
    cpf = input("Informe o CPF do cliente: ")
    cliente = filtrar_cliente(cpf, clientes)
    if not cliente:
        print("\n@@@ Cliente não encontrado! @@@")
    ...
    data_inicio = input("Informe a data de início do período (dd-mm-aaaa): ")
    data_fim = input("Informe a data de fim do período (dd-mm-aaaa): ")

    print(f"\n=== Extrato da conta no período de {data_inicio} a {data_fim} ===")
    transacoes = conta.historico.transacoes
    ...
    if not extrato:
        extrato = "Não há transações no período especificado."

    print(extrato)
    print("==================================================================")

    
def atualizar_limite_conta_corrente(contas):
    numero_conta = input("Informe o número da conta corrente: ")
    conta = next((conta for conta in contas if isinstance(conta, ContaCorrente) and conta.numero == numero_conta), None)
    if not conta:
        print("\n@@@ Conta corrente não encontrada! @@@")
        return

    novo_limite = float(input("Informe o novo limite da conta corrente: "))
    conta.limite = novo_limite
    print("\n=== Limite da conta corrente atualizado com sucesso! ===")
...
def menu():
    ...
    [t]\tTransferir
    [e]\tExtrato
    [ep]\tExtrato por período
    [c]\tConsultar saldo
    [nc]\tNova conta
    [lc]\tListar contas
    [nu]\tNovo usuário
    [el]\tAtualizar limite conta corrente
    [en]\tEncerrar conta
    [q]\tSair
    => """
    return input(textwrap.dedent(menu))
...
def main():
    ...
    while True:
        ...
        if opcao == "d":
            ...
            atualizar_limite_conta_corrente(contas)
        elif opcao == "en":
            encerrar_conta(clientes)
        elif opcao == "q":
            break
        else:
            print("\n")
